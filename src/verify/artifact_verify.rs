...
fn cid_hex(s: &str) -> Result<&str, String> {
  if !is_sha256(s) { return Err("M3_BAD_CID".into()); }
  Ok(&s[7..])
}
  ...
  fn canon_value(v: &serde_json::Value, out: &mut String) -> Result<(), String> {
    match v {
      ...
      serde_json::Value::Number(n) => {
        let s = n.to_string();
        if s.contains('+') { return Err("M3_CANON_NUM_PLUS".into()); }
        if s.starts_with('0') && s.len() > 1 && !s.starts_with("0.") { return Err("M3_CANON_NUM_LEADING_ZERO".into()); }
        if s.ends_with(".0") { return Err("M3_CANON_NUM_DOT0".into()); }
        out.push_str(&s);
        Ok(())
      }
...
fn expect_str<'a>(obj: &'a serde_json::Map<String, serde_json::Value>, k: &str) -> Result<&'a str, String> {
  obj.get(k).and_then(|v| v.as_str()).ok_or_else(|| format!("M3_EXPECT_STRING {}", k))
}
...
pub fn verify_artifact_outdir(outdir: &str) -> Result<(), String> {
  ...
  let digests_v: serde_json::Value = serde_json::from_slice(&digests_bytes).map_err(|_| "M3_DIGESTS_PARSE_FAIL".to_string())?;
  let files_obj = digests_v.get("files").and_then(|v| v.as_object()).ok_or_else(|| "M3_DIGESTS_MISSING_files".to_string())?;
  ...
  let graph_bytes = fs::read(&graph_p).map_err(|_| "M3_MISSING_artifact_graph.json".to_string())?;
  let graph_s = std::str::from_utf8(&graph_bytes).map_err(|_| "M3_GRAPH_NOT_UTF8".to_string())?;
  if graph_s.contains('\r') { return Err("M3_GRAPH_HAS_CR".into()); }
  if graph_s.ends_with(' ') || graph_s.ends_with('\t') { return Err("M3_GRAPH_TRAILING_SPACE".into()); }
  ...
  let graph_v: serde_json::Value = serde_json::from_slice(&graph_bytes).map_err(|_| "M3_GRAPH_PARSE_FAIL".to_string())?;
  let canon = canon_json(&graph_v)?;
  if canon != trimmed { return Err("M3_GRAPH_CANON_MISMATCH".into()); }

  let gobj = graph_v.as_object().ok_or_else(|| "M3_GRAPH_NOT_OBJECT".to_string())?;
  expect_only_keys(gobj, &["edges","nodes","v"])?;
  let vver = gobj.get("v").and_then(|v| v.as_str()).ok_or_else(|| "M3_GRAPH_MISSING_v".to_string())?;
  if vver != "0.1.0" { return Err("M3_GRAPH_BAD_v".into()); }

  let nodes = gobj.get("nodes").and_then(|v| v.as_array()).ok_or_else(|| "M3_GRAPH_MISSING_nodes".to_string())?;
  let edges = gobj.get("edges").and_then(|v| v.as_array()).ok_or_else(|| "M3_GRAPH_MISSING_edges".to_string())?;

  let mut node_cids: BTreeSet<String> = BTreeSet::new();

  for n in nodes {
    let no = n.as_object().ok_or_else(|| "M3_NODE_NOT_OBJECT".to_string())?;
    expect_only_keys(no, &["cid","name","role"])?;
    let cid = expect_str(no, "cid")?;
    if !is_sha256(cid) { return Err("M3_NODE_BAD_CID".into()); }
    let _name = expect_str(no, "name")?;
    let role = expect_str(no, "role")?;
    if !matches!(role, "in" | "out") { return Err("M3_NODE_BAD_ROLE".into()); }
    if !node_cids.insert(cid.to_string()) { return Err("M3_NODE_DUP_CID".into()); }
  }

  for e in edges {
    let eo = e.as_object().ok_or_else(|| "M3_EDGE_NOT_OBJECT".to_string())?;
    expect_only_keys(eo, &["from","kind","to"])?;
    let from = expect_str(eo, "from")?;
    let to = expect_str(eo, "to")?;
    let kind = expect_str(eo, "kind")?;
    if kind != "used_by" { return Err("M3_EDGE_BAD_KIND".into()); }
    if !node_cids.contains(from) { return Err("M3_EDGE_FROM_UNKNOWN".into()); }
    if !node_cids.contains(to) { return Err("M3_EDGE_TO_UNKNOWN".into()); }
  }
