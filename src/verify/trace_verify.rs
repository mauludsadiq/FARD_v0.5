  ...
  fn canon_value(v: &serde_json::Value, out: &mut String) -> Result<(), String> {
    match v {
      ...
      serde_json::Value::Number(n) => {
        ...
        if s.starts_with('0') && s.len() > 1 && !s.starts_with("0.") { return Err("CANON_NUM_LEADING_ZERO".into()); }
...
pub fn verify_trace_outdir(outdir: &str) -> Result<(), String> {
  ...
  let digests_v: serde_json::Value = serde_json::from_slice(&digests_bytes).map_err(|_| "M2_DIGESTS_PARSE_FAIL".to_string())?;
  let ok = digests_v.get("ok").and_then(|v| v.as_bool()).ok_or_else(|| "M2_DIGESTS_MISSING_ok".to_string())?;

  let trace_bytes = fs::read(&trace_p).map_err(|_| "M2_MISSING_trace.ndjson".to_string())?;
  let trace_str = std::str::from_utf8(&trace_bytes).map_err(|_| "M2_TRACE_NOT_UTF8".to_string())?;

  let allowed_t: BTreeSet<&str> = ["module_resolve","module_graph","artifact_in","artifact_out","error"].into_iter().collect();
  ...
  for (idx, raw_line) in trace_str.split('\n').enumerate() {
    ...
    let v: serde_json::Value = serde_json::from_str(raw_line).map_err(|_| "M2_TRACE_LINE_PARSE_FAIL".to_string())?;
    let obj = v.as_object().ok_or_else(|| "M2_TRACE_LINE_NOT_OBJECT".to_string())?;
  ...
  if error_count > 0 {
    if error_count != 1 { return Err("M2_ERROR_NOT_ONCE".into()); }
    if last_t.as_deref() != Some("error") { return Err("M2_ERROR_NOT_LAST".into()); }
  }
  ...
  if ok {
    if error_count != 0 { return Err("M2_OK_MUST_HAVE_NO_ERROR_EVENT".into()); }
    if !has_result { return Err("M2_OK_MUST_HAVE_result.json".into()); }
    if has_error { return Err("M2_OK_MUST_NOT_HAVE_error.json".into()); }
  } else {
    if error_count != 1 { return Err("M2_FAIL_MUST_HAVE_ONE_ERROR_EVENT".into()); }
    if has_result { return Err("M2_FAIL_MUST_NOT_HAVE_result.json".into()); }
    if !has_error { return Err("M2_FAIL_MUST_HAVE_error.json".into()); }
  }
