...
fn expect_only_keys(obj: &serde_json::Map<String, serde_json::Value>, allowed: &[&str]) -> Result<(), String> {
  ...
  for k in obj.keys() {
    if !allow.contains(k.as_str()) { return Err(format!("M4_EXTRA_KEY {}", k)); }
  }
  Ok(())
}

fn expect_str<'a>(obj: &'a serde_json::Map<String, serde_json::Value>, k: &str) -> Result<&'a str, String> {
  obj.get(k).and_then(|v| v.as_str()).ok_or_else(|| format!("M4_EXPECT_STRING {}", k))
}
...
fn expect_obj<'a>(obj: &'a serde_json::Map<String, serde_json::Value>, k: &str) -> Result<&'a serde_json::Map<String, serde_json::Value>, String> {
  obj.get(k).and_then(|v| v.as_object()).ok_or_else(|| format!("M4_EXPECT_OBJECT {}", k))
}

fn expect_arr<'a>(obj: &'a serde_json::Map<String, serde_json::Value>, k: &str) -> Result<&'a Vec<serde_json::Value>, String> {
  obj.get(k).and_then(|v| v.as_array()).ok_or_else(|| format!("M4_EXPECT_ARRAY {}", k))
}

pub fn parse_manifest_bytes(bytes: &[u8]) -> Result<(String, BTreeMap<String, Vec<String>>), String> {
  let v: serde_json::Value = serde_json::from_slice(bytes).map_err(|_| "M4_MANIFEST_PARSE_FAIL".to_string())?;
  let root = v.as_object().ok_or_else(|| "M4_MANIFEST_NOT_OBJECT".to_string())?;
  expect_only_keys(root, &["modules","v"])?;
  let ver = root.get("v").and_then(|x| x.as_str()).ok_or_else(|| "M4_MANIFEST_MISSING_v".to_string())?;
  if ver != "0.5-m4" { return Err("M4_MANIFEST_BAD_v".into()); }

  let modules = expect_obj(root, "modules")?;

  let mut out: BTreeMap<String, Vec<String>> = BTreeMap::new();
  for (mname, mv) in modules.iter() {
    let mo = mv.as_object().ok_or_else(|| "M4_MODULE_NOT_OBJECT".to_string())?;
    expect_only_keys(mo, &["exports"])?;
    let ex = expect_arr(mo, "exports")?;
    let mut exports: Vec<String> = Vec::new();
    for e in ex.iter() {
      let s = e.as_str().ok_or_else(|| "M4_EXPORT_NOT_STRING".to_string())?;
      exports.push(s.to_string());
    }
    if exports.is_empty() { return Err("M4_EXPORTS_EMPTY".into()); }
...
pub fn builtin_std_export_index() -> BTreeMap<String, BTreeSet<String>> {
  ...
  fn add(m: &mut BTreeMap<String, BTreeSet<String>>, module: &str, export: &str) {
    m.entry(module.to_string()).or_insert_with(BTreeSet::new).insert(export.to_string());
  }
...
pub fn assert_builtin_satisfies_manifest(
  ...
  for (mname, exports) in manifest.iter() {
    let b = builtin.get(mname).ok_or_else(|| format!("M4_BUILTIN_MISSING_MODULE {}", mname))?;
    for e in exports.iter() {
      if !b.contains(e) { return Err(format!("M4_BUILTIN_MISSING_EXPORT {}::{}", mname, e)); }
    }
  }
  Ok(())
}
