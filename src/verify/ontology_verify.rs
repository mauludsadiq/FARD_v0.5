...
use valuecore::json::{JsonVal, from_slice};

type Obj = BTreeMap<String, JsonVal>;

fn expect_only_keys(obj: &Obj, allowed: &[&str]) -> Result<(), String> {
...
fn expect_str<'a>(obj: &'a Obj, k: &str) -> Result<&'a str, String> {
    obj.get(k)
        .and_then(|v| v.as_str())
        .ok_or_else(|| format!("M4_EXPECT_STRING {}", k))
}

fn expect_obj<'a>(obj: &'a Obj, k: &str) -> Result<&'a Obj, String> {
    obj.get(k)
        .and_then(|v| v.as_object())
        .ok_or_else(|| format!("M4_EXPECT_OBJECT {}", k))
}

fn expect_arr<'a>(obj: &'a Obj, k: &str) -> Result<&'a Vec<JsonVal>, String> {
    obj.get(k)
...
pub fn parse_manifest_bytes(
    ...
    let v = from_slice(bytes).map_err(|_| "M4_MANIFEST_PARSE_FAIL".to_string())?;
    let root = v.as_object().ok_or_else(|| "M4_MANIFEST_NOT_OBJECT".to_string())?;
    expect_only_keys(root, &["modules", "v"])?;
    let ver = root.get("v").and_then(|x| x.as_str())
...
pub fn require_minimum_surface(mods: &BTreeMap<String, Vec<String>>) -> Result<(), String> {
    let req = [
        "std/result", "std/option", "std/list", "std/rec", "std/str",
        "std/json", "std/http", "std/fs", "std/path", "std/null",
        "std/int", "std/schema", "std/flow", "std/grow", "std/map",
        "std/time", "std/trace", "std/artifact", "std/hash",
    ];
    for m in req.iter() {
