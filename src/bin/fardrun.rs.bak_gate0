...
fn main() -> Result<()> {
    ...
    let mut tracer = Tracer::new(&trace_path)?;
    ...
    let v = loader.eval_main(&program, &mut tracer)?;
...
impl ModuleLoader {
    ...
    fn load_module(
        ...
        if self.stack.contains(&name.to_string()) {
            bail!("cycle detected in imports at {name}");
        }
        ...
        let exports = if name.starts_with("std/") {
            ...
            let path = here.join(format!("{name}.fard"));
    ...
    fn check_lock(&self, module: &str, got: &str) -> Result<()> {
        if let Some(lk) = &self.lock {
            if let Some(exp) = lk.expected(module) {
                if exp != got {
                    bail!("lock mismatch for module {module}: expected {exp}, got {got}");
                }
            }
        }
        Ok(())
