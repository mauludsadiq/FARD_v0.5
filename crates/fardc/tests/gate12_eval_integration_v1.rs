...
fn gate12_v1_eval_integration_produces_stable_runid() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_fardc"))
        .args(["--src", src.to_str().unwrap(), "--out", bundle_dir.to_str().unwrap()])
        .output()
        .expect("spawn fardc");
    assert!(out.status.success(), "fardc failed: {}", String::from_utf8_lossy(&out.stderr));
    ...
    let out = Command::new("cargo")
        ...
        .output()
        .expect("run abirun");
    assert!(out.status.success(), "abirun failed: {}", String::from_utf8_lossy(&out.stderr));
    ...
    const FROZEN_RUNID: &str = "sha256:8ed574172a09bc38fd8dd4f70d0f1d119083a10790bee6d00340cceb545e41e2";
