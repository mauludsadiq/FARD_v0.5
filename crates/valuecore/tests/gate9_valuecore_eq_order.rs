...
fn gate9_map_canon_eq_is_order_insensitive_recursive() {
    ...
    assert!(v0::canon_eq(&a, &b), "canon_eq must ignore map insertion order recursively");
    assert_eq!(v0::canon_cmp(&a, &b), Ordering::Equal, "canon_cmp must agree with canon_eq");

    let ea = v0::encode_json(&a);
    let eb = v0::encode_json(&b);
    assert_eq!(ea, eb, "canonical encoding must not depend on map insertion order");
}
...
fn gate9_canon_cmp_is_total_and_antisymmetric() {
    let vals: Vec<V> = vec![
        ...
        V::Map(vec![("b".to_string(), V::Int(2)), ("a".to_string(), V::Int(1))]),
        V::Map(vec![("a".to_string(), V::Int(1)), ("b".to_string(), V::Int(2))]),
    ];
    ...
    for i in 0..vals.len() {
        for j in 0..vals.len() {
            ...
            if ab == Ordering::Less {
                assert!(ba == Ordering::Greater, "antisymmetry failed: i={} j={}", i, j);
            } else if ab == Ordering::Greater {
                assert!(ba == Ordering::Less, "antisymmetry failed: i={} j={}", i, j);
            } else {
                assert!(ba == Ordering::Equal, "antisymmetry failed: i={} j={}", i, j);
            }
        }
    }
}
...
fn gate9_sorting_by_canon_cmp_is_deterministic_and_idempotent() {
    let mut xs: Vec<V> = vec![
        V::Map(vec![("z".to_string(), V::Int(1)), ("a".to_string(), V::Int(2))]),
        V::Map(vec![("a".to_string(), V::Int(2)), ("z".to_string(), V::Int(1))]),
        V::Int(0),
        V::Int(-1),
        V::Text("b".to_string()),
        V::Text("a".to_string()),
    ...
    for i in 1..xs.len() {
        ...
        assert!(c == Ordering::Less || c == Ordering::Equal, "not sorted at i={}", i);
    }
