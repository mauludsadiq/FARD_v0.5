...
fn gate11_rejects_unsorted_map_vector() {
    ...
    assert!(e.contains("non-canonical map order") || e.contains("ERROR_JSON"), "expected non-canonical order error: {}", e);
}

#[test]
fn gate11_rejects_duplicate_map_keys() {
    let e = must_err(br#"{"t":"map","v":[["a",{"t":"int","v":1}],["a",{"t":"int","v":2}]]}"#);
    assert!(e.contains("duplicate map key") || e.contains("ERROR_JSON"), "expected duplicate key error: {}", e);
}

#[test]
fn gate11_accepts_canonical_map_vector() {
    let v = v0::decode_json(br#"{"t":"map","v":[["a",{"t":"int","v":1}],["b",{"t":"int","v":2}]]}"#).unwrap();
    let j = v0::encode_json(&v);
    assert_eq!(std::str::from_utf8(&j).unwrap(), r#"{"t":"map","v":[["a",{"t":"int","v":1}],["b",{"t":"int","v":2}]]}"#);
}
