...
#[test]
fn m4_negative_missing_export_fails() {
    ...
    // delete one required export deterministically
    if let Some(s) = builtin.get_mut("std/result") {
        s.remove("ok");
    }
    ...
    assert!(msg.contains("std/result::ok"), "WRONG_ERR {}", msg);
}
