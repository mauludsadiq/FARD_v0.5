...
fn add_artifact_bundle(outdir: &str) -> String {
    ...
    fs::write(format!("{}/artifacts/{}.bin", outdir, hex), bytes)
        .expect("WRITE_ARTIFACT_BYTES_FAIL");

    let graph = format!(
    "{{\"edges\":[],\"nodes\":[{{\"cid\":\"{}\",\"name\":\"x\",\"role\":\"out\"}}],\"v\":\"0.1.0\"}}\n",
    cid
  );
    fs::write(format!("{}/artifact_graph.json", outdir), graph.as_bytes())
    ...
    let ev = format!(
        "{{\"cid\":\"{}\",\"name\":\"x\",\"t\":\"artifact_out\"}}\n",
        cid
    );
    trace.push_str(&ev);
    fs::write(&trace_p, trace.as_bytes()).expect("WRITE_TRACE_FAIL");

    let graph_hex = shasum256_bytes(
        fs::read(format!("{}/artifact_graph.json", outdir))
            .expect("READ_GRAPH_FAIL")
            .as_slice(),
    );
    ...
    let patched = dig.replacen(
        "{\"files\":{",
        &format!("{{\"files\":{{\"artifact_graph.json\":\"{}\",", graph_cid),
        1,
    );
