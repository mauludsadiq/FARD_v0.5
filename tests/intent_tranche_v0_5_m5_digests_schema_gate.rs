...
fn check(outdir: &str, expect_ok: bool) {
    ...
    if ok {
        assert!(
            ...
            "FILES_MISSING_result.json"
        );
        assert!(
            !files.contains_key("error.json"),
            "FILES_SHOULD_NOT_HAVE_error.json"
        );
    } else {
        assert!(files.contains_key("error.json"), "FILES_MISSING_error.json");
        assert!(
            !files.contains_key("result.json"),
            "FILES_SHOULD_NOT_HAVE_result.json"
        );
    }
    ...
    for (k, v) in files.iter() {
        let s = v
            .as_str()
            .unwrap_or_else(|| panic!("FILES_VALUE_NOT_STR {}", k));
